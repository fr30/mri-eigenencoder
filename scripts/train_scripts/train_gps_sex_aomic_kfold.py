...
from src.dataset import AOMICfMRIDataset, GPSConcatDataset
...
@hydra.main(version_base=None, config_path="configs", config_name="gps_sex_aomic")
def main(cfg):
    ...
    aomic_dataset = AOMICfMRIDataset(split="full", label="sex")
    labels = aomic_dataset.labels

    dataset = GPSConcatDataset([aomic_dataset])
    kf = StratifiedKFold(
        n_splits=5,
        shuffle=True,
