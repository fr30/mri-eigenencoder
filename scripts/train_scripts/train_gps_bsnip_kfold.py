...
from src.dataset import BSNIPfMRIDataset, GPSConcatDataset
from src.utils import EarlyStopping
from src.train import setup_experiment_gps, run_training_mc

warnings.filterwarnings("ignore", category=UserWarning)


@hydra.main(version_base=None, config_path="configs", config_name="gps_bsnip")
def main(cfg):
    ...
    dataset = GPSConcatDataset([bsnip_dataset])
    kf = StratifiedKFold(
        n_splits=5,
        shuffle=True,
        random_state=cfg.meta.seed,
    )
    labels = bsnip_dataset.labels
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        fold_acc.append(val_accs[max_epoch])
