...
class MCALoss:
    def __init__(self, emb_size, device):
        self.track_cov_final = torch.zeros((emb_size * 2, emb_size * 2), requires_grad=False).to(device)
        self.track_cov_estimate_final = torch.zeros((emb_size * 2, emb_size * 2), requires_grad=False).to(device)
        self.step = 0
    ...
    def cost_trace(RFG, track_cov_estimate_final, dim):
        ...
        RF_EI = torch.pinverse(RF_E)
        RG_EI = torch.pinverse(RG_E)
...
save_path = './BASELINE_UNSUPERVISED_PINV/'
