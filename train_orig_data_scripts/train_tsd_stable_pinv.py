...
def fmcat_loss(fmri_f, smri_f):
    ...
    tsd = -torch.trace(lhs @ rhs) / fmri_f.shape[0]

    return tsd

    # RF_I = torch.inverse(RF + eps)
    # RG_I = torch.inverse(RG + eps)
    # COST = (
    #     -RF_I.detach() @ RF @ RF_I.detach() @ P.detach() @ RG_I.detach() @ P.T.detach()
    #     + RF_I.detach() @ P @ RG_I.detach() @ P.T.detach()
    #     - RF_I.detach() @ P.detach() @ RG_I.detach() @ RG @ RG_I.detach() @ P.T.detach()
    #     + RF_I.detach() @ P.detach() @ RG_I.detach() @ P.T
    # )
...
# optimizer_1 = optim.RMSprop(NET_1.parameters(), lr=lr1)
# optimizer_2 = optim.RMSprop(NET_2.parameters(), lr=lr2)

# optimizer_1 = torch.optim.AdamW(
#     NET_1.parameters(), lr=lr1, weight_decay=1e-3
# )
# optimizer_2 = torch.optim.AdamW(
#     NET_2.parameters(), lr=lr2, weight_decay=1e-3
# )
...
track_cov_final = torch.zeros((256, 256)).cuda()
track_cov_estimate_final = torch.zeros((256, 256)).cuda()
